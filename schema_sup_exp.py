...
from components.dataset import Dataset as dataset
...
def get_new(train_1, train):
    new_list = []
    for e in train_1.examples:
        if e in train:
            continue
        new_list.append(e)
    return len(new_list)
...
def train(args):
    ...
    # decoder: 0.005 - 0.00005
    # encoder: 0.00005, 0.0001
    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_optimizer if not any(nd in n for nd in ['plmm_model'])], 'lr': args.lr},
        {'params': [p for n, p in param_optimizer if any(nd in n for nd in ['plmm_model'])], 'lr': args.lr * 0.05}
    ]

    #print(len(optimizer_grouped_parameters[0]['params']))
    #print(len(optimizer_grouped_parameters[1]['params']))
    if args.optimizer == 'RMSprop':
        optimizer = optimizer_cls(optimizer_grouped_parameters, lr=args.lr, alpha=args.alpha)
    else:
        optimizer = optimizer_cls(optimizer_grouped_parameters,betas=(0.9, 0.98), eps=1e-9, lr=args.lr)
    ...
    while True:
        ...
        for batch_examples in train_set.schema_batch_iter(batch_size=args.batch_size):
            train_iter += 1
            optimizer.zero_grad()
            ...
            if args.sup_attention:
                ...
                if att_probs is not None:
                    #print (len(att_probs))
                    #print(torch.stack(att_probs).size())
        ...
        if is_better and args.dev_file:
...
def test(args):
    ...
    parser.eval()
    evaluator = Registrable.by_name(args.evaluator)(args=args, vocab = parser.vocab)
    #print(len(test_set.examples))
    ...
    if args.save_decode_to:
        ...
        with open(args.save_decode_to, 'w') as f:
            for utterance, LF in eval_results['correct_pairs']:
                print(utterance)
                f.write(utterance.strip() + '\t' + LF.strip() + '\n')
        f.close()
    if args.evaluator == 'action_evaluator':
        ...
        print(eval_results['accuracy'], file=sys.stderr)
