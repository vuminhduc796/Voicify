...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        if mask is not None:
            mask = mask.unsqueeze(1).expand(attn.size())
            log_weight = self.dropout(masked_log_softmax(attn, mask, dim=-1))
            attn = attn.masked_fill(mask == 0, -1e9)
        else:
            log_weight = self.dropout(torch.log_softmax(attn, dim=-1))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q, log_attn = self.attention(q, k, v, mask=mask)
        ...
        return self.fc(q).squeeze(1), log_attn.squeeze(2)
