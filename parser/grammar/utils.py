...
def is_var(elem, dataset='geo_prolog', elem_set=set()):
    if dataset == 'geo_prolog':
        return elem.isupper() and len(elem) == 1
    elif dataset == 'job_prolog':
        return elem in ['ANS', 'X', 'A', 'B', 'P', 'J']
    elif dataset == 'geo_lambda' or dataset == 'atis_lambda' or dataset == 'question_lambda':
...
def is_lit(elem, dataset='geo_prolog', elem_set=set()):
    if dataset == 'geo_prolog':
        ...
        return re.match(r"\'(.+)\'", elem) and elem in elem_set

def get_type(elem, data_set = 'basketball', grammar_dict = dict()):
    if data_set == 'basketball':
        if elem in grammar_dict:
            return grammar_dict[elem]
        else:
            return False
def is_predicate(elem, dataset='geo_prolog', elem_set=set()):
    if dataset == 'geo_prolog':
        pass
    ...
    elif dataset == 'nlmap' or dataset == 'overnight_lambda':
        return not (elem in ['(', ')', ',', 'call-SW.listValue', 'call-SW.getProperty',
                             'lambda', 's', 'call-SW.filter', 'var',
                             'call-SW.ensureNumericProperty', 'string', 'call-SW.reverse', 'call-SW.ensureNumericEntity', 'call-SW.domain', 'number', 'date','time','query', 'keyval', 'qtype'] or elem.startswith('call-SW.'))
