...
def init_arg_parser():
    ...
    arg_parser.add_argument('--task_name', type=str, help='task name')
    ...
    arg_parser.add_argument('--backtran_file', type=str, help='path to the back translation file')

    arg_parser.add_argument('--english_file', type=str, help='path to the english file')

    arg_parser.add_argument('--decay_type', type=str, help='decay type')
    ...
    arg_parser.add_argument('--data_dir_prefix', type=str, help='data directory path')
    ...
    arg_parser.add_argument('--load_back_model', default=None, type=str, help='Load a back translated model')
    ...
    arg_parser.add_argument('--extra_cluster_ratio', default=1, type=float, help='Number of extra cluster ratio')
    ...
    arg_parser.add_argument('--plmm_model_name', choices=['xlm-roberta-base','roberta-base', 'facebook/bart-large', 'roberta-large', 'xlm-roberta-large'], default='xlm-roberta-base', help='Pre-trained model names')

    arg_parser.add_argument('--shuffle_tasks', default=True, action='store_true', help='Shuffle task or not')
    arg_parser.add_argument('--batch_decode', default=False, action='store_true', help='batch decode')
    ...
    arg_parser.add_argument('--sample_method', choices=['loss', 'random','f_cluster', 'gss', 'gem', 'max', 'normal', 'sum', 'ne_max', 'ne_sum', 'topk_sum', 'topk_max','init_max', 'init_sum', 'ne_topk_max', 'ne_topk_sum', 'init_min', 'ne_topk_min', 'entropy', 'ne_entropy', 'balance', 'label_uni', 'pro_max', 'pro_sum', 'pro_max_gem', 'pro_max_gem_sub', 'pro_max_gem_reverse','pro_max_gem_reverse_v1', 'pro_max_gem_reverse_v2', 'pro_max_gem_rescale', 'pro_max_gem_turn', 'IQP_uniform','gss_cluster','greedy_uniform', 'graph_clustering', 'IQP_graph',
                                                        ...
                                                        'lf_tfidf','length', 'back', 'back_gap', 'template_tfidf','grammar_cluster',
                                                        'diverse_cluster', 'diverse_cluster_gap',
                                                        'diverse_cluster_whole', 'diverse_cluster_entropy','diverse_cluster_integrate',
                                                        'integrate_tfidf', 'integrate_tfidf_model',
                                                        'integrate_tfidf_parse', 'uniform_template',
                                                        'backward_loss', 'nl_LM_feature', 'traffic',
                                                        'integrate_tfidf_parse_remove_tem', 'integrate_tfidf_parse_ikmeans',
                                                        'integrate_tfidf_parse_remove_cluster', 'score_select',
                                                        'score_select_remove_tem', 'score_select_gmm', 'IQP_select',
                                                        'IQP_select_gmm', 'RTTL','CSSE', 'random_no_mt','only_mt','only_ht','only_en', 'english_diversity', 'english_diversity_filter'], default='random', help='Sample Method')

    arg_parser.add_argument('--scaler', choices=['RobustScaler', 'MinMaxScaler', 'StandardScaler' , 'MaxAbsScaler', 'PowerTransformer' , 'Rank', 'box-cox', 'yeo-johnson', 'QuantileTransformer', 'QuantileTransformer_normal','QuantileTransformer_normal_minmax' , 'PowerTransformer_std'], default='RobustScaler', help='scaler')
    ...
    arg_parser.add_argument('--coff', type=float, default=0.5, help='coff term')

    arg_parser.add_argument('--coff2', type=float, default=0.5, help='coff term')
    arg_parser.add_argument('--coff3', type=float, default=0.5, help='coff term')

    arg_parser.add_argument('--bias_term', type=str, help='translation bias')

    arg_parser.add_argument('--error_term', type=str, help='translation error')
    arg_parser.add_argument('--density_term', type=str, help='prob density')

    arg_parser.add_argument('--diversity_term', type=str, default='kmeans', help='diversity term')
    ...
    arg_parser.add_argument('--annotation', type=int, default=0,
                        help='diversified annotation')
