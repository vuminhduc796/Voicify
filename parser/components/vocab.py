...
class GenNTVocabEntry(VocabEntry):
    def __init__(self):
        ...
        self.ent_action2nl = dict()
        self.action2nl = dict()

    def read_grammar(self, file_path, data_type, lang='overnight'):
        if lang == 'overnight':
            self.vertex2action = {k.vertex.to_lambda_expr: k for k, v in self.token2id.items()}
        elif lang == 'nlmap':
            self.vertex2action = {k.vertex.to_prolog_expr: k for k, v in self.token2id.items()}
        else:
            raise ValueError
        ...
        with open(full_path) as grammar_file:
            for line in grammar_file:
                if line.strip():
                    ...
                    for nl in line_split[2:]:
                        nl_set.add(' '.join([nltk_lemmer.lemmatize(token) for token in nl.split(' ')]))
                    if (lhs == '$EntityNP' or lhs == '$LOC' or lhs == '$POI') and rhs in self.vertex2action:

                        action = self.vertex2action[rhs]
                        action.type = action.get_vertex_type()
                        entype = action.type
                        if action not in self.ent_action2nl:
                            self.ent_action2nl[action] = []

                        self.ent_action2nl[action] = list(nl_set)
